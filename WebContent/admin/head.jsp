<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>

<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
    <head>
        <meta http-equiv=content-type content="text/html; charset=utf-8" />
        <link href="../css/admin.css" type="text/css" rel="stylesheet" />
        <script>
           function quit(){
        	   
        	   var s=confirm("确认推出吗");
        	 return s;
        	   
           }
        </script>
    </head>
    <body>
        <table cellspacing=0 cellpadding=0 width="100%" 
               background="../img/header_bg.jpg" border=0>
            <tr height=56>
                <td width=260><img height=56 src="../img/header_left.jpg" 
                                   width=260></td>
                <td style="font-weight: bold; color: #fff; padding-top: 20px" 
                    align=middle>当前用户:${sessionScope.admin["admin_name"]} &nbsp;&nbsp; <a style="color: #fff" 
                                                        href="${pageContext.request.contextPath}/admin/toupdatepsd.action" 
                                                        target="_top">修改口令</a> &nbsp;&nbsp; <a style="color: #fff" 
                                                        onclick="return quit()" 
                                                        href="${pageContext.request.contextPath}/admin/exitlogin.action" target=_top>退出系统</a> 
                </td>
                <td align=right width=268><img height=56 
                                               src="../img/header_right.jpg" width=268></td></tr></table>
        <table cellspacing=0 cellpadding=0 width="100%" border=0>
            <tr bgcolor=#1c5db6 height=4>
                <td></td>
            </tr>
        </table>
    </body>
</html>